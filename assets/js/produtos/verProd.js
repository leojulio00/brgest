...
import { getDatabase, ref, set, child, onValue, get, remove } from 'https://www.gstatic.com/firebasejs/9.14.0/firebase-database.js';
...
var precVendaRDetalheProduto = document.querySelector(".precVendaRDetalheProduto")
var precCompraDetalheProduto = document.querySelector(".precCompraDetalheProduto")
var lucroVendaDetalheProduto = document.querySelector(".lucroVendaDetalheProduto")
var quantProdEDetalheProduto = document.querySelector('.quantProdEDetalheProduto')
var dataValidadeDetalheProduto = document.querySelector(".dataValidadeDetalheProduto")
...
var btnCloseDetalhesprodutos = document.querySelector('.btnCloseDetalhesprodutos')
var btnEditarProduto = document.querySelector('.btnEditarProduto')
var btnSalvarAlteracoesProduto = document.querySelector('.btnSalvarAlteracoesProduto')
...
function AddDetalhesProdModal(nomeProd, precCompra, precVenda, quantiProd, lucroProd, dataValidade, horaReg){
    ...
    'Nome: ' + nomeProd + '</br>' + '</br>' +
    'Preço de compra: ' + precCompra + ' ' + TipoMoeda + '</br>' + '</br>' +
    'Preço de venda: ' + precVenda + ' ' + TipoMoeda + '</br>' + '</br>' +
    'Lucro por unidade: ' + lucroProd + ' ' + TipoMoeda + '</br>' + '</br>' +
    'Quantidade no estoque: ' + quantiProd + '</br>' + '</br>' +
    'Data de validade: ' + dataValidade + '</br>' + '</br>'
...
function addDetalhesInputs(nomeProd, precCompra, precVenda, quantiProd, lucroProd, dataValidade){
...
function addItemToTable(nomeProd, quantiProd, dataValidade, dataModi, precVenda, precCompra, lucroProd, horaReg){
...
precVendaRDetalheProduto.addEventListener("keyup", ()=>{
    valorLucroVenda = Math.abs(precCompraDetalheProduto.value - precVendaRDetalheProduto.value)
    ...
    divCard.addEventListener('click', ()=>{
        ...
        AddDetalhesProdModal(nomeProd, precCompra, precVenda, quantiProd, lucroProd, dataValidade, horaReg)

        addDetalhesInputs(nomeProd, precCompra, precVenda, quantiProd, lucroProd, dataValidade)

        
        btnSalvarAlteracoesProduto.addEventListener('click', ()=>{
            try {
                set(ref(db, '/estabelecimentos/' + usuarioEstabelecimento + '/produtos/todosProdutos/' + nomeProd + '/precCompra'), precCompraDetalheProduto.value);
                set(ref(db, '/estabelecimentos/' + usuarioEstabelecimento + '/produtos/todosProdutos/' + nomeProd + '/precVenda'), precVendaRDetalheProduto.value);
                set(ref(db, '/estabelecimentos/' + usuarioEstabelecimento + '/produtos/todosProdutos/' + nomeProd + '/lucroProd'), lucroVendaDetalheProduto.value);
                set(ref(db, '/estabelecimentos/' + usuarioEstabelecimento + '/produtos/todosProdutos/' + nomeProd + '/quantProdE'), quantProdEDetalheProduto.value);
                set(ref(db, '/estabelecimentos/' + usuarioEstabelecimento + '/produtos/todosProdutos/' + nomeProd + '/dataValidade'), dataValidadeDetalheProduto.value);
        ...
        btnDeletarProduto.addEventListener('click', ()=>{
            try {
                remove(ref(db, '/estabelecimentos/' + usuarioEstabelecimento + '/produtos/todosProdutos/' + nomeProd))
...
function addAllItemsToTable(produtosEstoque){
    ...
    produtosEstoque.forEach(element => {
        addItemToTable(element.nomeProd, element.quantProdE, element.dataValidade, element.horaRegEst, element.precVenda, element.precCompra, element.lucroProd, element.horaReg)
    });
}
